...
export function createCastle(scene, position = new THREE.Vector3(0, 0, 0), rotation = 0) {
    ...
    function createWallSegment(width, depth, height, posX, posZ, rotY = 0, mat = materialMuralha) {
    ...
    function createWallWithDoor(wallWidth, wallHeight, thickness, doorWidth, doorHeight, posX, posZ, rotY = 0, mat = materialMuralha) {
    ...
    function createWallWithDoors(wallWidth, wallHeight, thickness, doors, posX, posZ, rotY = 0, mat = materialMuralha, baseY = 0) {
        const shape = new THREE.Shape();
        const hw = wallWidth / 2;
    ...
    function createHollowRoundTower(radius, height, posX, posZ, segments = 8, t = 1.0, mat = materialTorre) {
    ...
    function createHollowSquareTower(sizeX, sizeZ, height, posX, posZ, t = 1.0, mat = materialTorre) {
        const halfX = sizeX / 2;
        const halfZ = sizeZ / 2;
        const halfT = t / 2;
        ...
        createWallSegment(sizeX, t, height, posX, posZ + halfZ - halfT, 0, mat);
        createWallSegment(t, sizeZ - 2 * t, height, posX - halfX + halfT, posZ, 0, mat);
        createWallSegment(t, sizeZ - 2 * t, height, posX + halfX - halfT, posZ, 0, mat);
    }

    function createStairs(width, totalHeight, totalDepth, numSteps, startX, startZ, directionZ = 1, mat = materialMadeira) {
    ...
    function createBattlementsAlongX(length, posX, posZ, topY, mat = materialAmeia, merlonW = 0.9, merlonH = 0.6, gap = 0.9, merlonDepth = 1.0) {
    ...
    function createBattlementsAlongZ(length, posX, posZ, topY, mat = materialAmeia, merlonW = 0.9, merlonH = 0.6, gap = 0.9, merlonDepth = 1.0) {
        const step = merlonW + gap;
        const count = Math.max(1, Math.floor((length + gap) / step));
        const usedLength = count * step - gap;
    ...
    function createRoundBattlements(radius, posX, posZ, topY, segments = 16, mat = materialAmeia) {
        const angleStep = (Math.PI * 2) / segments;
    ...
    function createSquareBattlements(sizeX, sizeZ, topY, posX, posZ, mat = materialAmeia) {
    ...
    function createSquareCap(sizeX, sizeZ, topY, posX, posZ, mat = materialTorre) {
    ...
    createHollowSquareTower(westTowerSize, westTowerSize, westTowerHeight, -halfW, 0);
    createSquareBattlements(westTowerSize, westTowerSize, westTowerHeight, -halfW, 0);
    ...
    createWallSegment(offsetDistance + wallThickness, wallThickness, wallHeight, halfW + offsetDistance / 2, -halfD + 12.5, 0, materialMuralha);

    createWallSegment(wallThickness, 9.0, wallHeight, halfW + offsetDistance, -halfD + 17.0, 0, materialMuralha);

    createWallSegment(offsetDistance + wallThickness, wallThickness, wallHeight, halfW + offsetDistance / 2, -halfD + 21.5, 0, materialMuralha);

    createWallSegment(wallThickness, 5.0, wallHeight, halfW, -halfD + 24.0);

    createHollowSquareTower(eastTowerSize, eastTowerSize, eastTowerHeight, halfW, 5.0);
    createSquareBattlements(eastTowerSize, eastTowerSize, eastTowerHeight, halfW, 5.0);
...

    {
        ...
        createWallSegment(northSegmentLength, wallThickness, wallHeight, posX1, -halfD);
    }

    const gateTowerLeftX = -(gateOpening / 2 + gateTowerW / 2);
    createHollowSquareTower(gateTowerW, gateTowerD, gateTowerH, gateTowerLeftX, -halfD);
    createSquareBattlements(gateTowerW, gateTowerD, gateTowerH, gateTowerLeftX, -halfD);
    createSquareCap(gateTowerW, gateTowerD, gateTowerH, gateTowerLeftX, -halfD);
    createArrowSlit(gateTowerLeftX, -halfD - gateTowerD / 2 - 0.1, 0, gateTowerH * 0.55);
    ...
    createHollowSquareTower(gateTowerW, gateTowerD, gateTowerH, gateTowerRightX, -halfD);
    createSquareBattlements(gateTowerW, gateTowerD, gateTowerH, gateTowerRightX, -halfD);
    createSquareCap(gateTowerW, gateTowerD, gateTowerH, gateTowerRightX, -halfD);
    createArrowSlit(gateTowerRightX, -halfD - gateTowerD / 2 - 0.1, 0, gateTowerH * 0.55);

    {
        const posX2 = halfW - cornerRadius - northSegmentLength / 2;
        createWallSegment(northSegmentLength, wallThickness, wallHeight, posX2, -halfD);
    }
...

    {
        ...
        createWallSegment(southSegmentLength, wallThickness, wallHeight, posX3, halfD);
    }
...

    {
        ...
        createWallSegment(southSegmentLength, wallThickness, wallHeight, posX4, halfD);
    }
    ...
    createWallWithDoors(
        ...
        partitionThickness,
        [
            { offsetX: -innerHalfW / 2, width: interiorDoorWidth, height: interiorDoorHeight }, // porta da sala esquerda (oeste)
            { offsetX: innerHalfW / 2, width: interiorDoorWidth, height: interiorDoorHeight },  // porta da sala direita (leste)
        ],
        0,
        hallSouthZ
    );
    ...
    floorShape.lineTo(innerHalfW + offsetDistance, -halfD + 12.5 - wallThickness / 2);
    floorShape.lineTo(innerHalfW + offsetDistance, -halfD + 21.5 + wallThickness / 2);
    ...
    const secondFloorGeo = new THREE.ExtrudeGeometry(floorShape, extrudeSettings2);
    ...
    createStairs(stairWidth, secondFloorHeight, stairDepth, numSteps, stairX_W, stairStartZ, 1);
    createStairs(stairWidth, secondFloorHeight, stairDepth, numSteps, stairX_E, stairStartZ, 1);
